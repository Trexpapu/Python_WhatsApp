...
def formulario():
    ...
    v_formulario.boton_regresar = v.tk.Button(v_formulario, text="Regresar al menu", command=lambda: regresar(v_formulario), font = fuente_personalizada, width=20, height=5, bg = "#009aff")
    v_formulario.boton_regresar.place(x=ancho-ancho + 40, y=alto-alto+20)


    #enviar datos
    v_formulario.boton_enviar = v.tk.Button(v_formulario, text="Subir datos", command=lambda : enviar(entry_Nombre.get(), entry_Celular.get()), font = fuente_personalizada, width=20, height=5, bg = "#BBD631")
