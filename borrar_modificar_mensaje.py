...
def eliminarRegistro(id_a_eliminar):
    ...
    try:
        ...
        cursor.execute("DELETE FROM MENSAJES WHERE ID = ?", (id_a_eliminar,))
...
def modificarRegistro(id_, anio_, mes_, dia_, mensaje_):
    ...
    try:
        ...
        cursor.execute("UPDATE MENSAJES SET MENSAJE = ?, ANIO = ?, MES = ?, DIA = ? WHERE ID = ?", (mensaje_, anio_, mes_, dia_, id_))
        # Confirma los cambios y cierra la conexión
        conexion.commit()
...
def borrarOeliminar(id, celular, anio, mes, dia, mensaje):
    ...
    #label celular
    label_celular = v.tk.Label(ventana_formulario, text=f"Celular: {celular}", font=fuente_personalizada, width=28, height=3, bg="#009aff", relief="sunken", bd=2)
    ...
    boton_modificar = v.tk.Button(ventana_formulario, text="Modificar", command=lambda: modificarRegistro(id, entry_anio.get(), entry_mes.get(), entry_dia.get(), entry_mensaje.get()), width=8, height=4, bg="#66BB6A", font=fuente_personalizada)    
